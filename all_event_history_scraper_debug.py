    ...
    with sync_playwright() as p:
        ...
        for i, event in enumerate(EVENTS):
            ...
            try:
                ...
                page.wait_for_selector("table.genTbl.openTbl.ecHistoryTab", timeout=15000)
                page.screenshot(path=f"screenshot_{i+1}_{event['name'][:10].replace(' ', '_')}.png", full_page=True)
                time.sleep(1)

                rows = page.query_selector_all("table.genTbl.openTbl.ecHistoryTab tbody tr")
                print(f"📊 Found {len(rows)} rows for {event['name']}")
